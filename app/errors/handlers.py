...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def app_error_handler(request: Request, exc: AppError):
        # 记录异常日志，结构化日志格式
        logger.warning(
            "event=app_error method=%s path=%s code=%s http_status=%s message=%s",
            request.method,
            request.url.path,
            exc.code,
            exc.http_status,
    ...
    async def validation_exception_handler(
        ...
        logger.info(
            "event=validation_error method=%s path=%s message=%s",
            request.method,
            request.url.path,
    ...
    async def integrity_error_handler(request: Request, exc: IntegrityError):
        # IntegrityError
        logger.warning(
            "event=db_integrity_error method=%s path=%s",
            request.method,
            request.url.path,
        )
    ...
    async def sqlalchemy_error_handler(request: Request, exc: SQLAlchemyError):
        ...
        logger.exception(
            "event=db_error method=%s path=%s",
            request.method,
            request.url.path,
        )
        return JSONResponse(
    ...
    # HTTPException - 处理HTTP异常，比如404
    @app.exception_handler(StarletteHTTPException)
    async def http_exception_handler(request: Request, exc: StarletteHTTPException):
        logger.info(
            "event=http_error method=%s path=%s status=%s detail=%s",
            request.method,
            request.url.path,
            exc.status_code,
            exc.detail,
